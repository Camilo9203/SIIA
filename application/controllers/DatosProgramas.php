	...
	public function create()
	{
		if ($this->input->post()) {
			...
			if (in_array($programaIngresado, $programas)) {
				echo json_encode(array('status' => "warning", 'msg' => "Programa: " . $programaIngresado .  " ya registrado."));
			}
			else {
				if ($this->db->insert('datosProgramas', $data)){
					...
					echo json_encode(array('status' => "error", 'msg' => "Ocurrió un error y no se guardaron los datos del programa seleccionado."));
				}
			}
		} else {
			echo json_encode(array('status' => "error", 'msg' => "Verifique los datos ingresado, no se están enviando las variables tipo post necesarios."));
		}
	}
	...
	function enviomail_aceptar_programas ($organizacion, $data){
		...
		if($this->email->send()) {
			...
			if($this->db->insert('correosregistro', $correo_registro)){
				echo json_encode(array('status' => "success", 'msg' => "Se envío un correo a: " . $organizacion->direccionCorreoElectronicoOrganizacion . ", por favor verifíquelo"));
			}
			else {
				echo json_encode(array('status'=> 'info','msg' => "Se envío el correo de activación pero no se guardo registro en base de datos"));
			}
