...
class Formularios extends CI_Controller
{
	...
	public function guardar_formulario_informacion_general_entidad()
	{
		...
		if ($this->form_validation->run("formulario_informacion_general_entidad") == FALSE) {
			...
			echo json_encode(array('url' => "panel", 'msg' => $error, 'status' => 0));
		}
		else {
			if ($this->input->post()) {
				...
				if ($datos_informacion_general != NULL) {
					...
					echo json_encode(array('url' => "panel", 'msg' => "Se actualizó la Información General.", 'status' => 1));
					...
					echo json_encode(array('url' => "panel", 'msg' => "Se guardo la Información General.", 'status' => 1));
					$this->logs_sia->session_log('Formulario Información General');
					$this->logs_sia->logQueries();
				}
			} else {
				echo json_encode(array('url' => "panel", 'msg' => "Verifique los datos ingresado, no son correctos.", 'status' => 1));
			}
		}
	}
	// Formulario 2
