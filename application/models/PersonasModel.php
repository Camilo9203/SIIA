...
class PersonasModel extends CI_Model
{
	...
	public function personasCapacitadas($id = FALSE)
	{
		if ($id === FALSE) {

			// Consulta para traer organizaciones acreditas
			$query = $this->db->select("*")->from("historialPersonasCapacitadas")->get();
			return $query->result_array();
		}
		// Traer solicitudes por id
		$query = $this->db->get_where('historialPersonasCapacitadas', array('id' => $id));
